...
const PARTICLE_CONFIG: Record<ParticleType, ParticleConfig> = {
  rock: { color: "#FF6B6B", label: "Rock", emoji: "🪨", defeats: "scissors" },
  paper: { color: "#4ECB71", label: "Paper", emoji: "📄", defeats: "rock" },
  scissors: {
    color: "#4A9DEC",
    label: "Scissors",
    emoji: "✂️",
...
const ParticleSystem = () => {
  ...
  const predictWinner = (counts: ParticleCounts) => {
  ...
  const initializeSimulation = () => {
    ...
    // Create engine and renderer with no gravity
    ...
    // Helper function to create a particle with random velocity
    const createParticle = (type: ParticleType, x: number, y: number) => {
      const angle = Math.random() * Math.PI * 2;
      const velocity = 1.5 + Math.random(); // Random velocity between 1.5 and 2.5

      const particle = Matter.Bodies.circle(x, y, config.particleSize, {
        restitution: 0.9 + Math.random() * 0.2, // Random bounciness between 0.9 and 1.1
        friction: 0,
        frictionAir: 0,
        frictionStatic: 0,
        density: 0.1,
        ...
        label: type,
        // Store initial speed as custom property
        plugin: {
    ...
    // Create initial particles with adjusted spacing
    ...
    const updateParticleCounts = () => {
      ...
      setParticleCounts((prev) => {
        ...
        if (newCounts.length > MAX_DATA_POINTS) {
    ...
    Matter.Events.on(engine, "collisionStart", (event) => {
      event.pairs.forEach((pair) => {
        ...
        const transformParticle = (
          loser: Matter.Body,
          winner: ParticleType
        ) => {
          // Only change the color and type, maintain current velocity
          loser.render.fillStyle = PARTICLE_CONFIG[winner].color;
          loser.label = winner;
        };
        ...
        if (typeA !== typeB) {
          if (
            ...
            transformParticle(bodyB, typeA);
          } else {
            transformParticle(bodyA, typeB);
          }
    ...
    // Maintain constant speed for all particles
    Matter.Events.on(engine, "afterUpdate", () => {
      ...
      if (particleTypes.size === 1) {
        const winnerType = Array.from(particleTypes)[0] as ParticleType;
  ...
  useEffect(() => {
    updateSimulationSpeed(config.simulationSpeed);
  }, [config.simulationSpeed]);

  useEffect(() => {
    // Don't auto-start the simulation
    return () => {
  ...
  const handlePredictionSelect = (type: ParticleType) => {
    ...
    <div className="w-full max-w-7xl mx-auto">
        ...
        <div className="bg-zinc-900 rounded-2xl p-8 max-w-xl w-full mx-4 border border-white/10">
          <div className="relative">
            ...
            {step === 0 && (
              <div className="space-y-8 animate-fadeIn">
                <div className="text-center">
                  <h2 className="text-2xl font-medium text-white mb-4">Welcome to Particle Battle!</h2>
                  <p className="text-white/60 mb-8">Watch particles compete in an epic game of Rock, Paper, Scissors!</p>
                </div>

                <div className="grid grid-cols-3 gap-4">
                  {(Object.keys(PARTICLE_CONFIG) as ParticleType[]).map((type) => (
                    <div key={type} className="flex flex-col items-center gap-2 p-4 rounded-xl bg-zinc-800/50 border border-white/5">
                      <span className="text-3xl">{PARTICLE_CONFIG[type].emoji}</span>
                      <span className="text-white/90">{PARTICLE_CONFIG[type].label}</span>
                      <span className="text-white/60 text-sm">defeats {PARTICLE_CONFIG[PARTICLE_CONFIG[type].defeats].emoji}</span>
                    </div>
                  ))}
                </div>
            ...
            {step === 1 && (
              <div className="space-y-6 animate-fadeIn">
                <div className="text-center mb-8">
                  <h2 className="text-xl font-medium text-white mb-2">Configure Your Particles</h2>
                  <p className="text-white/60">Adjust the counts and size to create your perfect battle!</p>
                </div>
                    ...
                    <div key={type} className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-2xl">{PARTICLE_CONFIG[type].emoji}</span>
                          <span className="text-white/90">{config.particleCounts[type]}</span>
                        </div>
                      </div>
                      <input
                        type="range"
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-white/90">{config.particleSize}</span>
                      </div>
                      <input
                        type="range"
                        min="5"
            ...
            {step === 2 && (
              <div className="space-y-6 animate-fadeIn">
                <div className="text-center">
                  <h3 className="text-xl font-medium text-white mb-2">Make Your Prediction</h3>
                  <p className="text-white/60">Based on the particle counts, who do you think will win?</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="rounded-lg bg-zinc-900 border border-white/10 p-6">
            ...
            {userPrediction && (
              <div className="mt-4 p-4 rounded-lg bg-zinc-800/50 border border-white/5">
                <div className="text-sm text-white/60 mb-2">Your Prediction</div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">{PARTICLE_CONFIG[userPrediction].emoji}</span>
                    <span className="text-white/90">{PARTICLE_CONFIG[userPrediction].label}</span>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="lg:col-span-2 space-y-8">
          {/* Simulation Canvas */}
          <div ref={containerRef} className="relative rounded-lg overflow-hidden bg-zinc-900 border border-white/10">
            <canvas ref={canvasRef} className="w-full h-auto" />
            {winner && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80">
                <div className="text-center mb-8">
                  <div className="text-6xl mb-4">{PARTICLE_CONFIG[winner].emoji}</div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData}>
                ...
                <XAxis 
                  ...
                  label={{ value: 'Time', position: 'insideBottom', offset: -5, fill: 'rgba(255,255,255,0.6)' }}
                />
                <YAxis 
                  stroke="rgba(255,255,255,0.6)"
                  label={{ value: 'Count', angle: -90, position: 'insideLeft', fill: 'rgba(255,255,255,0.6)' }}
                />
                <Tooltip 
                  contentStyle={{ 
                ...
                />
                <Bar dataKey="rock" stackId="a" fill="#ef4444" name="Rock 🪨" />
                <Bar dataKey="paper" stackId="a" fill="#22c55e" name="Paper 📄" />
                <Bar dataKey="scissors" stackId="a" fill="#3b82f6" name="Scissors ✂️" />
                <Line 
                  type="monotone" 
                  dataKey="total" 
                  stroke="#ffffff" 
